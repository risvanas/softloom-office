                                    ...
                                    <script>
                                        <?php foreach ($leave_details as $key => $value) { ?>
                                            $(document).ready(function() {
                                                addleave("<?php echo $value->LEAVE_DATE ?>","<?php echo $value->LEAVE_TYPE ?>","<?php echo str_replace("&#39;", "'", $value->REASON) ?>","<?php echo $value->PM_STATUS ?>","<?php echo $value->HR_STATUS ?>", "<?php echo $value->ID ?>","<?php echo $value->PM_COMMENT ?>","<?php echo $value->HR_COMMENT ?>")
                                            })
...
<script>
    ...
    function addleave(date, leave_type, reason, pm, hr, id, pm_cmnt, hr_cmnt)
    {
//        console.log(obj)
//        return;
//        date = $(obj).val();
...
//        $("#leave_type" + Numx).append($("<option></option>").val('0.5').html("Half day"));
//        $("#leave_type" + Numx).append($("<option></option>").val('1').html("Full day"));
        ...
        leave_type = (leave_type == 1) ? "Full day" : "Half day";
        $("#leave_type" + Numx).val(leave_type);
        $("#leave_reason" + Numx).val(reason);
        $("#pm_aprov" + Numx).val(pm);
        $("#pm_comment" + Numx).val(pm_cmnt);
        console.log(pm + " **** " +  hr);
        
        if(pm != "") {
            $("#pm_aprov" + Numx).attr("disabled",true)
            $("#pm_comment" + Numx).attr("disabled",true)
        }
        $("#hr_aprov" + Numx).val(hr);
        $("#hr_comment" + Numx).val(hr_cmnt);
        if(hr != "") {
            $("#hr_aprov" + Numx).attr("disabled",true)
            $("#hr_comment" + Numx).attr("disabled",true)
        }
