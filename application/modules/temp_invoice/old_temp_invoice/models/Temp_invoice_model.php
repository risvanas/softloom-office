...
class Temp_invoice_model extends CI_Model {
    ...
    function multipe_select($dat_from, $dat_to, $key_words, $inv_type, $inv_status) {
        ...
        if($inv_status != 'paid') {
            ...
            $this->db->where("tbl_temp_invoice.BOOK_NUMBER IN(SELECT TEMP_INVOICE_ID FROM tbl_invoice WHERE ACC_YEAR_CODE=11 and COMPANY=1 AND TEMP_INVOICE_ID IS NOT NULL)");
        }
        if($inv_type != "") {
            $this->db->where("tbl_temp_invoice.INVOICE_TYPE", $inv_type);
        } else {
            $this->db->where("tbl_temp_invoice.INVOICE_TYPE ='with_tax'");
        }
        $this->db->where("tbl_temp_invoice.COMPANY", $company);
...
//			$this->db->where('tbl_temp_invoice.INVOICE_DATE >=',$dat_from);
//			$this->db->where('tbl_temp_invoice.INVOICE_DATE <=',$dat_to);
//			$this->db->like('tbl_temp_invoice.BOOK_NUMBER',$key_words);
//			
//			//return $result=$this->db->get();
//		}
...
//		}
        ...
        $this->db->order_by("tbl_temp_invoice.BOOK_NUMBER", "desc");
