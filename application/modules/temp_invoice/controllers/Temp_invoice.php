...
//            render_template($layout);
    ...
    function set_invoice_as_paid(){
        $temp_inv_no = $this->uri->segment(3);
        $temp_data = array(
                'INVOICE_STATUS' => 1
            );
            ...
            redirect('temp_invoice/invoice_list');
    }
...
//        $generate_pdf = $this->input->post('generate_pdf');
        ...
        $data_id = str_replace("-", "=", $data_id);
        $data_id = str_replace(".", "+", $data_id);
        $invoice_no = $this->encryption->decrypt($data_id);
        //$book_num= $this->uri->segment(3);
        $data['vno'] = $this->temp_invoice_model->invoice_edit($invoice_no,$accounting_year, $company);
...
//        }
    ...
    function invoice_edit() {
        ...
        $data_id = str_replace("-", "=", $data_id);
        $data_id = str_replace(".", "+", $data_id);
        $temp_inv_id = $this->encryption->decrypt($data_id);
        //$book_num= $this->uri->segment(3);
        $data['vno'] = $this->temp_invoice_model->invoice_edit($temp_inv_id, $accounting_year, $company);
        $data['sub_acc_list'] = $this->temp_invoice_model->select_sub_acc('tbl_account');
        $data['cust'] = $this->temp_invoice_model->select_customer('tbl_account');
        $data['description'] = $this->temp_invoice_model->select_description();
        $data['msg'] = 'Invoice added successfully';
...
//        $acc_year = $val2['ACC_YEAR_CODE'];
        ...
        $location_details = $this->lockdate->location_details();
        $create_on = gmdate("Y-m-d H:i:s");
        if ($flood_cess_percent =='')
        {
        	$flood_cess_percent=0;
        }
        if ($flood_cess_amt =='')
        {
        	$flood_cess_amt=0;
...
//        render_template($layout);
    ...
    function invoice_delete() {
        ...
        $data_id = str_replace(".", "+", $data_id);
        $temp_inv_id = $this->encryption->decrypt($data_id);
        $query2 = $this->db->query("select ACC_YEAR_CODE,INVOICE_DATE from tbl_temp_invoice where INVOICE_ID =$temp_inv_id AND DEL_FLAG=1 and  ACC_YEAR_CODE=$acc_year");
        $val2 = $query2->row_array();
        ...
        if ($year_code == $acc_year && $check_status == 'false') {
            $data['vno'] = $this->temp_invoice_model->invoice_edit($temp_inv_id, $acc_year, $company);
            $data['sub_acc_list'] = $this->temp_invoice_model->select_sub_acc('tbl_account');
            $data['cust'] = $this->temp_invoice_model->select_customer('tbl_account');
            $data['description'] = $this->temp_invoice_model->select_description();
            $data['msg'] = '';
