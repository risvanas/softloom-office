...
<body>
    ...
    foreach ($fee_details->result() as $row) {
        ...
        $query = $this->db->query("SELECT * FROM tbl_accounting_year WHERE AY_ID='$row->ACC_YEAR_CODE'");
        $res = $query->row_array();
        $year_code_from = $res['FROM_DATE'];
        $year_code_to = $res['TO_DATE'];
        $year_code_from = date("y", strtotime($year_code_from));
        $year_code_to = date("y", strtotime($year_code_to));
    }
    ?>
    <div id="header">
        <table class="table">
