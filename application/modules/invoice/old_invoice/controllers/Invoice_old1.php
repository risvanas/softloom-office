...
class Invoice extends MX_Controller
{
	...
	function invoice_update()
	{
		  ...
		  $this->lockdate->index($invoice_date);
			...
			render_template($layout);
			 ...
			 $query2=$this->db->query("select ACC_YEAR_CODE from tbl_transaction where BOOK_NUMBER=$book_num AND BOOK_NAME='INVOE' AND DEL_FLAG=1");
	...
	function invoice_delete()
	{
        ...
        $data_id = str_replace( ".", "+",  $data_id );
		$book_num= $this->encryption->decrypt($data_id);
		$query2=$this->db->query("select ACC_YEAR_CODE,DATE_OF_TRANSACTION from tbl_transaction where BOOK_NUMBER=$book_num AND BOOK_NAME='INVOE' AND DEL_FLAG=1");
               	$val2=$query2->row_array();
                $acc_year=$val2['ACC_YEAR_CODE']; 
				$this->load->library('../controllers/lockdate');
