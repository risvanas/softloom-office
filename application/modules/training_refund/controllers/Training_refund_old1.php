...
FROM tbl_transaction WHERE BOOK_NAME='RFD'");  
	...
	function fee_edit()
	{
		...
		function fee_update()
    		 ...
    		 $this->lockdate->index($pay_date);
			 $check_status=$this->lockdate->check_date($pay_date);
			 $message_display=$this->lockdate->check_date($pay_date,$check_status);
			 if($check_status == 'true')
			 {
				 $data['parent_account']=$this->trainingrefund_model->selectAll('tbl_account');
	     $data['r']=$this->trainingrefund_model->select_st_name('tbl_student');
		...
		$query2=$this->db->query("select ACC_YEAR_CODE from tbl_transaction where BOOK_NUMBER=$book_num AND BOOK_NAME='RFD' AND DEL_FLAG=1");
	...
	function fee_delete()
	{
		$sess_array=$this->session->userdata('logged_in');
				 $year_code=$sess_array['accounting_year'];
		 echo $id = $this->uri->segment(3);
		$query2=$this->db->query("select ACC_YEAR_CODE,DATE_OF_TRANSACTION from tbl_transaction where BOOK_NUMBER=$id AND BOOK_NAME='RFD' AND DEL_FLAG=1");
               	$val2=$query2->row_array();
                $acc_year=$val2['ACC_YEAR_CODE']; 
				$this->load->library('../controllers/lockdate');
