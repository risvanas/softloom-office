...
class Graph extends MX_Controller {
    ...
    function index() {
        ...
        $current_financial_year = $this->db->query("SELECT * FROM tbl_accounting_year WHERE DEL_FLAG=1 AND STATUS='active'")->row();
        $date2 = $current_financial_year->TO_DATE;
        $date1 = $current_financial_year->FROM_DATE;
        $date2 = date('Y-m-d');
...
//            Month(tbl_transaction.DATE_OF_TRANSACTION) Order By YEAR, MONTH");
        $data['sal_summary'] = $this->graph_model->get_sales_summary($date1, $date2);
        $data['sal_return'] = $this->graph_model->get_sales_return($date1, $date2);
        $data['dev_return'] = $this->graph_model->get_dev_return($date1, $date2);
...
//            From tbl_transaction Where tbl_transaction.DEL_FLAG = 1 and tbl_transaction.DATE_OF_TRANSACTION >='$date1' and tbl_transaction.DATE_OF_TRANSACTION <= '$date2' Group By Year(tbl_transaction.DATE_OF_TRANSACTION),
//            Month(tbl_transaction.DATE_OF_TRANSACTION) Order By YEAR, MONTH");
//        $data['financial_year'] = $this->db->query("SELECT * FROM `tbl_accounting_year` WHERE DEL_FLAG=1");
//        $data['calendar_year'] = $this->db->query("Select DISTINCT Year(tbl_transaction.DATE_OF_TRANSACTION) As YEAR From tbl_transaction WHERE DEL_FLAG=1 Order By YEAR ASC");
    ...
    function payment_graph() {
        ...
        $this->permition_checker->permition_viewprocess($menu_id);
        $current_financial_year = $this->db->query("SELECT * FROM tbl_accounting_year WHERE DEL_FLAG=1 AND STATUS='active'")->row();
        $to_date = $current_financial_year->TO_DATE;
        $from_date = $current_financial_year->FROM_DATE;
...
//        $to1 = date('Y-M-d');
        ...
        $data['rtyp'] = "PAYMENT";
        $data['financial_year'] = $this->db->query("SELECT * FROM `tbl_accounting_year` WHERE DEL_FLAG=1");
        $data['calendar_year'] = $this->db->query("Select DISTINCT Year(tbl_transaction.DATE_OF_TRANSACTION) As YEAR From tbl_transaction WHERE DEL_FLAG=1 Order By YEAR");
