        ...
        foreach ($data_pass->result() as $res) {
            ...
            <tr>
                ...
                <td>
                    ...
                    $encrypted_data3 = str_replace("=", "-", $encrypted_data3);
                    $encrypted_data3 = str_replace("+", ".", $encrypted_data3);
                    $encrypted_data4 = $company_code;
                    $encrypted_data4 = $this->encryption->encrypt($encrypted_data4);
                    $encrypted_data4 = str_replace("/", "~", $encrypted_data4);
                    $encrypted_data4 = str_replace("=", "-", $encrypted_data4);
                    $encrypted_data4 = str_replace("+", ".", $encrypted_data4);
                    ?>
                    <a href="<?php echo site_url('trialbalance/view_details'); ?>/<?php echo $encrypted_data1; ?>/<?php echo $encrypted_data2; ?>/<?php echo $encrypted_data3; ?>/<?php echo $encrypted_data4; ?>" target="_blank"><?php echo $acc_name; ?></a></td>
       <!-- <td> --> <?php
...
foreach ($data_pass->result() as $res) {
    ...
    $openings = floatval($open['opening']) + floatval($op_balance); //echo $res->op;
    $total_opening+=$openings;
    ...
    $query1 = $this->db->query("SELECT SUM( DEBIT ) as debit,SUM( CREDIT ) AS credit
			 	...
			 	WHERE tbl_transaction.DEL_FLAG =  '1'
			  	...
			  	AND tbl_transaction.DATE_OF_TRANSACTION >= '$from_date' and tbl_transaction.DATE_OF_TRANSACTION <= '$next_date'");
    $debit_credit = $query1->row_array();
    $my_debit = floatval($debit_credit['credit']);
    $my_credit = floatval($debit_credit['debit']);
    ...
    $new_total = (floatval($openings) + floatval($my_credit)) - floatval($my_debit);
    if ($new_total >= 0) {
    ...
    $new1_total+=floatval($new1);
    $new2_total+=floatval($new2);
