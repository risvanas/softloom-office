                                ...
                                <div class="panel panel-default">
                                    ...
                                    <div class="panel-body panel-scroll" style="height:280px">
                                        <ul class="todo">
                                            ...
                                            $sql1 = "SELECT ANY_VALUE(abc.ACC_YEAR_CODE) AS ACC_YEAR_CODE, ANY_VALUE(abc.INVOICE_ID) AS INVOICE_ID, abc.CUSTOMER_ID, ANY_VALUE(abc.INVOICE_TYPE) AS INVOICE_TYPE, ANY_VALUE(abc.COMPANY) AS COMPANY, ANY_VALUE(abc.BOOK_NUMBER) AS BOOK_NUMBER, ANY_VALUE(abc.BOOK_NAME) AS BOOK_NAME, ANY_VALUE(abc.INVOICE_DATE) AS INVOICE_DATE, abc.DESCRIPTION, ANY_VALUE(abc.INVOICE_RECURRING) AS INVOICE_RECURRING, ANY_VALUE(abc.INVOICE_RECURRING_TYPE) AS INVOICE_RECURRING_TYPE, ANY_VALUE(abc.INVOICE_RECURRING_COMMENT) AS INVOICE_RECURRING_COMMENT, MAX(abc.NEXT_INVOICE_DATE) AS NEXT_INVOICE_DATE, ANY_VALUE(abc.REC_COMPLETE_FLAG) AS REC_COMPLETE_FLAG, ANY_VALUE(abc.ACC_NAME) AS ACC_NAME FROM (SELECT ACC_YEAR_CODE, INVOICE_ID, CUSTOMER_ID, INVOICE_TYPE, COMPANY, BOOK_NUMBER, BOOK_NAME, INVOICE_DATE, DESCRIPTION, INVOICE_RECURRING, INVOICE_RECURRING_TYPE, INVOICE_RECURRING_COMMENT, NEXT_INVOICE_DATE, REC_COMPLETE_FLAG, tbl_account.ACC_NAME FROM tbl_invoice JOIN tbl_account ON tbl_account.ACC_ID=tbl_invoice.CUSTOMER_ID WHERE tbl_invoice.DEL_FLAG=1 AND REC_COMPLETE_FLAG=1 AND NEXT_INVOICE_DATE < " . $this->db->escape($date) . " AND NEXT_INVOICE_DATE!=0 AND NEXT_INVOICE_DATE!='1970-01-01' UNION ALL SELECT ACC_YEAR_CODE, INVOICE_ID, CUSTOMER_ID, INVOICE_TYPE, COMPANY, BOOK_NUMBER, BOOK_NAME, INVOICE_DATE, DESCRIPTION, INVOICE_RECURRING, INVOICE_RECURRING_TYPE, INVOICE_RECURRING_COMMENT, NEXT_INVOICE_DATE, REC_COMPLETE_FLAG, tbl_account.ACC_NAME FROM tbl_temp_invoice JOIN tbl_account ON tbl_account.ACC_ID=tbl_temp_invoice.CUSTOMER_ID WHERE tbl_temp_invoice.DEL_FLAG=1 AND REC_COMPLETE_FLAG=1 AND NEXT_INVOICE_DATE < " . $this->db->escape($date) . " AND NEXT_INVOICE_DATE!=0 AND NEXT_INVOICE_DATE!='1970-01-01') abc GROUP BY abc.CUSTOMER_ID, abc.DESCRIPTION ORDER BY NEXT_INVOICE_DATE DESC";
