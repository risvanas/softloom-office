...
class Customer_payment extends MX_Controller
{
	...
	function index()
	{
		...
		if($temp_voc_num!="")
			{
			 $query2=$this->db->query("select ACC_YEAR_CODE from tbl_transaction where BOOK_NUMBER=$temp_voc_num AND BOOK_NAME='PAYD' AND DEL_FLAG=1");
	...
	function fee_delete()
	{
		...
		$query2=$this->db->query("select ACC_YEAR_CODE,DATE_OF_TRANSACTION from tbl_transaction where BOOK_NUMBER=$id AND BOOK_NAME='PAYD' AND DEL_FLAG=1");
               	$val2=$query2->row_array();
                $acc_year=$val2['ACC_YEAR_CODE']; 
				$this->load->library('../controllers/lockdate');
