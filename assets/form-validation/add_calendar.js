...
function myRowObject(zero, one, two, three, four, five, six) {
    this.zero = zero;
    this.one = one;
    this.two = two;
    this.three = three;
...
function appendRow() {
    ...
    el1.setAttribute('id', 'calendar_date' + iteration);
    el1.setAttribute('name', 'calendar_date' + iteration);
...
//		el1.setAttribute('size', '30');
    ...
    el2.setAttribute('id', 'calendar_day' + iteration);
    el2.setAttribute('name', 'calendar_day' + iteration);
...
//    el2.setAttribute('value', '');
    ...
    el3.setAttribute('onchange', 'update_working_hr(this)');
    el3.id = "calendar_off-" + iteration;
    el3.name = "calendar_off" + iteration;
    el3.align = "center";
    ...
    el4.setAttribute('id', 'working_hrs' + iteration);
    el4.setAttribute('name', 'working_hrs' + iteration);
//    el4.setAttribute('size', '15');
    el4.setAttribute('value', '1');
    el4.setAttribute('class', 'form-control cal_working_hrs');
//		el4.readOnly = false;
    ...
    el5.setAttribute('id', 'description' + iteration);
    el5.setAttribute('name', 'description' + iteration);
//    el4.setAttribute('size', '15');
    el5.setAttribute('value', '1');
...
//		el6.id="calendar_detail_id" + iteration;
//		el6.name="calendar_detail_id" + iteration;
//		cell5.appendChild(el6);	


    row.myRow = new myRowObject(el0, el1, el2, el3, el4, el5);
}
...
function reorderRows(tbl, startingIndex) {
    if (hasLoaded) {
        ...
        for (var i = startingIndex; i < tbl.rows.length; i++) {
            ...
            tbl.rows[i].myRow.one.name = 'calendar_date' + count; // input text
            tbl.rows[i].myRow.one.id = 'calendar_date' + count; // input text

            tbl.rows[i].myRow.two.name = 'calendar_day' + count; // input text
            tbl.rows[i].myRow.two.id = 'calendar_day' + count; // input text

            tbl.rows[i].myRow.three.name = 'calendar_off' + count; // input text
            tbl.rows[i].myRow.three.id = 'calendar_off' + count; // input text

            tbl.rows[i].myRow.four.name = 'working_hrs' + count; // input text
            tbl.rows[i].myRow.four.id = 'working_hrs' + count; // input text

            tbl.rows[i].myRow.five.name = 'description' + count; // input text
            tbl.rows[i].myRow.five.id = 'description' + count; // input text

//            tbl.rows[i].myRow.five.name = 'chkbox' + count; // input text
//            tbl.rows[i].myRow.five.id = 'chkbox' + count; // input text

//            tbl.rows[i].myRow.five.name = 'calendar_detail_id' + count; // input text
//            tbl.rows[i].myRow.five.id = 'calendar_detail_id' + count; // input text
