...
{	if (hasLoaded) {
		//if (tbl.thead[0].rows[startingIndex]) {
			...
			for (var i=startingIndex; i<tbl.rows.length; i++) {
					...
					tbl.rows[i].myRow.three.name = 'sub_acc' + count; // input text
					tbl.rows[i].myRow.three.id = 'sub_acc' + count; // input text
					
					tbl.rows[i].myRow.four.name = 'amount' + count; // input text
					tbl.rows[i].myRow.four.id = 'amount' + count; // input text
					
					tbl.rows[i].myRow.five.name = 'remarks' + count; // input text
					tbl.rows[i].myRow.five.id = 'remarks' + count; // input text
								
					tbl.rows[i].myRow.six.name = 'chkbox' + count; // input text
					tbl.rows[i].myRow.six.id = 'chkbox' + count; // input text
					
					tbl.rows[i].myRow.seven.name = 'acc_id' + count; // input text
					tbl.rows[i].myRow.seven.id = 'acc_id' + count; // input text
